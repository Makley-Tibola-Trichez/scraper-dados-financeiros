...
def inserir_acao_db(connection: Connection, acao: AcaoModel) -> None:
    cursor = connection.cursor()
    cursor.execute(
        """
...
def inserir_dividendos_anuais_db(connection: Connection, dividendos: list[DividendoAnualModel]) -> None:
    cursor = connection.cursor()
